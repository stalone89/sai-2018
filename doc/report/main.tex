...
\par Inside the loop longitude, latitude, altitude and True Air Speed values are printed and stored in a structure named Waypoint. The function \mintinline{c}{csv_waypoint_parse(char line[])} store the waypoints values inside an structure vector named \mintinline{c}{Waypointlist[]}.    
...
The function \mintinline{c}{sscanf} was chosen to store the coordinates data values.
