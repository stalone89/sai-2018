...
int main (void){
	...
	double v_tas, v_m, theta_true, theta_sensor, heading, distance, runtime_delta, csignal;

	double integral_acc = 0;
	double delay_memory = 0;

	FILE *fid_log = create_log("lab1_3.log");
	FILE *fid_errors = create_log("lab1_3_errors.log");
	...
	position_current_sensor = waypoint_prev_coor;
	...
	for(i = 0; i < waypoint_count - 1; i++){
		...
		do{
			...
			csignal = speed_controller(v_m, waypoint_list[i].tas, &integral_acc);
